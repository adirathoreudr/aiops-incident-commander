...
from agent.prompts.incident_prompt import build_human_prompt, parse_agent_response
...
class TestParseAgentResponse:
    ...
    def test_confidence_clamped_above_1(self):
        result = parse_agent_response(
            '{"confidence_score": 1.5, "incident_type": "x", "probable_root_cause": "x", "supporting_evidence": [], "recommended_action": "notify_only", "recommended_action_params": {}, "requires_approval": false, "approval_reason": ""}'
        )
        assert result["confidence_score"] == 1.0

    def test_confidence_clamped_below_0(self):
        result = parse_agent_response(
            '{"confidence_score": -0.2, "incident_type": "x", "probable_root_cause": "x", "supporting_evidence": [], "recommended_action": "notify_only", "recommended_action_params": {}, "requires_approval": false, "approval_reason": ""}'
        )
        assert result["confidence_score"] == 0.0
...
class TestBuildHumanPrompt:
            ...
            "logs": [
                {
                    ...
                    "level": "fatal",
                    "message": "FATAL: missing env var STRIPE_KEY",
                }
            ],
            "rollout_events": [
                {
                    ...
                    "status": "complete",
                    "started_at": "2026-04-19T09:58:00Z",
                }
            ],
        }
    ...
    def test_similar_incidents_included(self):
        similar = [
            {
                "title": "Past CrashLoop",
                "probable_root_cause": "Bad config",
                "resolved_by": "rollout_restart",
            }
        ]
